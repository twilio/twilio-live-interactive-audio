...
class ConversationParticipant: LiveStreamAudience {
    ...
    var isHandRaised: Bool {
        ...
        set {
            let dictionary = [handRaisedKey: newValue]
            participant.setAttributes(TCHJsonAttributes(dictionary: dictionary), completion: nil)
        }
    }
    private let participant: TCHParticipant
    private let handRaisedKey = "hand_raised"

    init(participant: TCHParticipant) {
